...
import { useIsMobile } from '@pfinance/ui/hooks/use-mobile'
...
import {
  ...
  SheetHeader,
  SheetTitle,
} from '@pfinance/ui/components/sheet'
import { Skeleton } from '@pfinance/ui/components/skeleton'
import { Tooltip, TooltipContent, TooltipTrigger } from '@pfinance/ui/components/tooltip'
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          'relative w-(--sidebar-width) bg-transparent transition-[width] duration-200 ease-move',
      ...
      <div
        ...
        data-side={side}
        className={cn(
          'fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-move data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] md:flex',
...
function SidebarInset({ className, ...props }: React.ComponentProps<'main'>) {
  return (
    <main
      ...
      className={cn(
        // min-w-0: a flex item's minimum width is its content's, so without
        // it a wide table pushes the whole main column past the viewport
...
function SidebarGroupLabel({
  ...
  return useRender({
    ...
    props: mergeProps<'div'>(
      {
        className: cn(
          'flex h-8 shrink-0 items-center rounded-lg px-3 text-xs font-medium text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-move group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
          className,
        ),
      },
      props,
    ...
    props: mergeProps<'button'>(
      {
        ...
        // The active nav entry is otherwise only styled (data-active); this
        // announces it to assistive tech too.
